...
\textbf{Three pillars of observability:} \textit{Metrics} (quantitative numbers over time), \textit{Logs} (qualitative event records), \textit{Traces} (follow one request across services via \textit{spans} to find latency/bottlenecks, e.g. Tempo, Jaeger). \\
...
\textbf{node-exporter:} hardware/OS metrics. \textbf{kube-state-metrics:} state of K8 objects, e.g. \texttt{kube\_deployment\_status\_replicas\_ready\{namespace="x",deployment="y"\}}. \\
