...
\textbf{Hierarchy:} a \kf{playbook} is one YAML file holding 1..n \kf{plays}; a play maps a group of \kf{hosts} to an ordered list of \kf{tasks}; each task is a single \kf{module} call (e.g. \kf{apt}, \kf{copy}), the atomic unit of work. \\
\textbf{Run:} \texttt{ansible-playbook -i inv playbook.yml} (\texttt{-i} = inventory file). Handy flags:
\begin{itemize}
  \item \kf{--check}, dry run: report what \textit{would} change, change nothing
  \item \kf{--list-tasks}, list the tasks without running them (preview)
  \item \kf{--tags <t>} / \kf{--skip-tags <t>}, run only / skip tasks tagged \texttt{<t>}
  \item \kf{--ask-vault-pass}, prompt for the Vault password (decrypt vaulted vars)
\end{itemize}
...
\end{lstlisting}
\textbf{Handlers:} special tasks that run \textit{only when notified} (by a task reporting \texttt{changed}) and \textit{once}, at the \textbf{end of the play} -- even if notified by many tasks. Ideal for ``restart/reload after a config change''. Triggered with \texttt{notify: <handler name>}; a failed play skips pending handlers unless \texttt{force\_handlers: true}.
...
- name: suboptimal              # one apt/yum call per item
  ...
  loop: "{{ packages }}"
\end{lstlisting}
\subsection{Modules, State, Ad-hoc \& Errors}
\textbf{Modules:} the units of work; most are \textit{idempotent} -- you declare the desired \texttt{state:} and the module changes reality only if needed, returning \texttt{ok}/\texttt{changed}/\texttt{failed} as JSON. Common: \texttt{apt}/\texttt{yum} (pkgs), \texttt{copy}/\texttt{template} (files), \texttt{service}/\texttt{systemd}, \texttt{file} (dirs/links), \texttt{user}, \texttt{git}, \texttt{uri}. \\
\textbf{State:} Ansible is \textit{stateless} -- no state file (unlike TF). Idempotency comes from each module comparing the desired \texttt{state} (\texttt{present}/\texttt{absent}/\texttt{started}/\texttt{stopped}/\texttt{latest}, \dots) to reality on every run, so re-running converges. \texttt{--check} = dry run, \texttt{--diff} = show what changes. \\
