...
\textbf{Terraform vs. Ansible:} Terraform \textit{provisions} infrastructure (create VMs, networks); Ansible \textit{configures} the provisioned machines (install \& configure software, deploy code). Often combined. Ansible is \textit{agentless} \& \textit{push-based}: over SSH it copies the generated Python module to a temp dir (\texttt{\textasciitilde/.ansible/tmp}) on the target, runs it with the remote Python, returns the result as JSON on stdout, then deletes it, nothing stays installed.
...
Once encrypted the file is just ciphertext, so committing it to Git is safe \& intended -- only the vault password must stay out of the repo.
