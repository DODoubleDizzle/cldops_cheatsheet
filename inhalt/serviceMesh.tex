...
\textbf{Pros:} + transparent (no app code changes, language-agnostic), + per-pod isolation (a proxy's config/failure is scoped to its pod), + platform team owns cross-cutting concerns (mTLS, retries, routing, telemetry). \\
\textbf{Cons:} - one proxy per pod $\rightarrow$ CPU/RAM scale with pod count, - extra network hop adds latency, - operational overhead (sidecar injection, proxy upgrade needs a pod restart), - overkill for few services.
