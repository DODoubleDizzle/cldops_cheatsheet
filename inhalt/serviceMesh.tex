...
\textcolor{ForestGreen}{\textbf{Pros:}} independent deploy, flexibility, scalability, fault isolation, team autonomy. \\
\textcolor{red}{\textbf{Cons:}} operational complexity, distributed-system challenges, data consistency, higher resource use. \\
\textbf{Challenges:} \kf{Communication:} services must know endpoints, new service ripples changes. \kf{Security:} in-cluster traffic unsecured, any pod $\to$ any pod. \kf{Monitoring:} spread out, logic must be added per app.
...
\textbf{Capabilities:} \kf{Traffic Mgmt} (routing, splitting), \kf{Resilience} (fault tolerance, retries, fallback), \kf{Security} (mTLS, authN/Z), \kf{Observability} (metrics, logs, tracing). \\
...
\textbf{Components:} \kf{Data Plane:} lightweight proxies (Envoy), handle comm/metrics/security. \kf{Control Plane:} manages config/policies (Istiod), pushes rules to data plane.
...
\textbf{Leading meshes:} \kf{Istio} (Google, feature-rich), \kf{Linkerd} (lightweight, perf), \kf{Kuma} (CNCF, VMs+containers), \kf{Consul Connect} (HashiCorp, discovery), \kf{Cilium} (eBPF, not dedicated mesh).

\subsection{Sidecar Pattern Pros/Cons}
\textcolor{ForestGreen}{\textbf{Pros:}} + transparent (no code change, lang-agnostic), + per-pod isolation, + platform team owns cross-cutting (mTLS/retries/routing/telemetry). \\
\textcolor{Red}{\textbf{Cons:}} - CPU/RAM scales with pod count, - extra hop = latency, - sidecar injection + proxy upgrade needs restart, - overkill for few services.
...
\textbf{Slicing:} \kf{Secure Overlay (L4):} TCP routing, mTLS tunneling, simple authZ, TCP metrics. \kf{L7 Processing:} HTTP routing/LB, circuit breaking, rate limiting, fault injection, retry, timeouts, rich authZ, HTTP metrics/access logs/tracing.
...
\textbf{Rolling:} gradual pod replace, default K8s. \textbf{Recreate / Big-bang:} kill all, deploy new (downtime). \textbf{Canary:} small \% to v2, watch, ramp up. \textbf{Blue/Green:} two envs, switch traffic atomically, easy rollback. \textbf{Dark Launching / Shadow:} mirror prod traffic to v2, drop response (test under real load). See \kf{General Definitions}.
